...
| `@jdsalasc/solvejs-async` | `sleep`, `timeout`, `retry`, `pMap`, `debouncePromise`, `throttlePromise`, `createTaskQueue`, `createRateLimiter`, `createTokenBucketLimiter`. | Integrated in monorepo flow and meta package. | None critical identified. | Burst controls are now available; add guidance for weighted token costs by endpoint tier. | Add per-endpoint token-cost cookbook examples. |
